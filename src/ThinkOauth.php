...
abstract class ThinkOauth {
	...
	public static function getInstance($type, $token = null) {
		...
		if (class_exists($class)) {
			...
			throw new \Exception('_CLASS_NOT_EXIST_' . ':' . $name);
		}
	}
	...
	//登录成功
	public function suceess($suceessFuc) {
		$code = $_GET['code'];
		if (empty($code)) {
			return $this;
		}
		...
		$token = $this->getAccessToken($code, $extend);

		if (empty($token)) {
			$this->error('参数错误');
		}
		$this->OauthMsg = array('TOKEN' => $token, 'TYPE' => $type, 'OPENID' => $token['openid'], 'ACCESS_TOKEN' => $token['access_token']);
		if (!empty($suceessFuc)) {
			$suceessFuc($this->OauthMsg);
		}
		$this->hasRun = true;
		return $this;
	}
	public function getOauthMsg() {
		return $this->OauthMsg;
	}
	...
	protected function http2($post = null, $url, $method = 'GET') {
	...
	protected function http($url, $params, $method = 'GET', $header = array(), $multi = false) {
		...
		curl_close($ch);
		if ($error) {
			//$data = $this->http2($postData, $url, $method);
